...
### Q: 슬롯은 어떻게 채워지나?
- 검사가 실행될 때마다 빈 슬롯 중 가장 낮은 번호에 결과 저장
...
### Q: 수동 실행은 언제 쓰나?
- 라벨 제거/푸시로 1회 자동 실행 후, 추가 검사가 필요할 때
...
- 쿨다운은 현재 커밋(SHA) 기준. 푸시하면 새 커밋이므로 쿨다운 초기화됨

### Q: 쿨다운 중에 검사를 실행하고 싶으면?
...
- 단, 검사 통과로 success가 된 경우는 복원 안 함 (override로 success된 경우만 복원)
...
- 예: feature → main (검사 대상)
- 예: feature → develop (검사 대상 아님)
